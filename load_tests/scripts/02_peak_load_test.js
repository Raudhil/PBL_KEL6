...
function checkIuran(baseUrl, headers) {
  ...
  // Get transactions
  let transaksiRes = http.get(
    `${baseUrl}${config.endpoints.transaksiIuran}?limit=20`,
    {
      headers: headers,
...
function viewPengumuman(baseUrl, headers) {
  ...
  let pengumumanRes = http.get(
    `${baseUrl}${config.endpoints.pengumuman}?limit=15`,
    {
      headers: headers,
  ...
  // View kegiatan
  let kegiatanRes = http.get(
    `${baseUrl}${config.endpoints.kegiatan}?limit=10`,
    {
      headers: headers,
