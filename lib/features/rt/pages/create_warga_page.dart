...
class _CreateWargaPageState extends ConsumerState<CreateWargaPage> {
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // VALIDASI: Cek apakah KK sudah ada Kepala Keluarga
        if (_selectedPeranKeluarga == 'Kepala Keluarga') {
          final supabaseService = SupabaseService();
          final existingKepala = await supabaseService.getKepalaKeluargaByKK(
            kkId,
          );

          if (existingKepala != null) {
            setState(() {
              _isLoading = false;
            });
            if (mounted) {
              _showKepalaKeluargaExistsDialog(
                existingKepala['nama_lengkap'] as String,
                existingKepala['nik'] as String,
              );
            }
        ...
        final newWarga = WargaModel(
          ...
          fotoKtp: null,
          // TODO: Uncomment setelah migration SQL dijalankan
        ...
        if (mounted) {
          // Use AppConfig untuk control error display
          final errorMessage = AppConfig.getErrorMessage(
            'Terjadi kesalahan saat menyimpan data warga. Silakan coba lagi.',
            e,
          );

          _showErrorDialog('Gagal Menyimpan Data', errorMessage);
        }
        AppConfig.logError('Submit Warga', e);
      }
    }
  }
  ...
  void _showKepalaKeluargaExistsDialog(String namaKepala, String nikKepala) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return Dialog(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  'KK ini sudah memiliki Kepala Keluarga:',
                  style: TextStyle(
                    fontSize: 14,
                    color: AppColors.textSecondary,
                  ),
                ...
                Text(
                  'Silakan pilih peran keluarga lain atau edit data Kepala Keluarga yang sudah ada.',
                  style: TextStyle(
                    fontSize: 13,
                    color: AppColors.textSecondary,
                  ),
